clear()
load("research/interactions/data/MC_final_results.Rdat")
...
source("research/interactions/R/multi_way_interaction_plot_gen.R")



pdf("research/interactions/writing/knitr/bias_correction.pdf")
head(results_corrected)
m = mw.interaction.plot(y="Estimate", sort.vars=c("skew","cor", "c"), smallest.to.biggest=T, FUN=mean, d=results_corrected, add=F, col=rgb(0,0,0,.25), ylim=c(-.2, .2), line.loc=c(0,3.25), label.points=T, sort.by.effects=F, pch=16)
abline(h=0, col="lightgray")
m = mw.interaction.plot(y="Estimate", sort.vars=c("skew","cor", "c"), smallest.to.biggest=T, FUN=mean, d=caseiii_results, add=T, label.points=F, sort.by.effects=F, pch=1)
