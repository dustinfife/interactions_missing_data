...
for (i in 1:nrow(bias.mat)){
	...
	##### record differences interaction term estimates
	bias.mat$interaction.coef[i] = coef(mod.full)[4]-coef(mod.rest)[4]
	
	##### determine how slopes/intercepts are affected
...
aggregate(interaction.coef~standardize, FUN=mean, data=bias.mat)


##### I'M STANDARDIZING BEFORE SELECTION HAPPENS! I NEED TO STANDARDIZE AFTER, DUDE!
